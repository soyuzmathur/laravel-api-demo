...
<!---Single Item-->
<li>
    <v-list-item class="rounded-s-xl" :to="item.to" rounded :disabled="item.disabled" :target="item.type === 'external' ? '_blank' : ''">
        <v-list-item-title class="v-list-item__prepend">{{item.title }}</v-list-item-title>
    </v-list-item>
</li>
</template>
<style scoped>
...
.v-list-item--active::before, .v-list-item--active::after{
...
/* .v-list-item--active .v-list-item-title::before {
    content: var(--tw-content);
    position: absolute;
